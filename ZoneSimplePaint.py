...
if __name__ == '__main__':
    ...
    pol = [MyPoint(Fraction(240), Fraction(210)),
           MyPoint(Fraction(280), Fraction(230)),
           MyPoint(Fraction(260), Fraction(250)),
           MyPoint(Fraction(220), Fraction(250))]
    pol = [point * MyCipher(3) - MyPoint(Fraction(0), Fraction(100)) for point in pol]
    DrawMyPolygon(screen, pol, (0, 0, 0), 3)
    for i in range(0, 4):
        DrawRayMyPoint(screen, pol[i], pol[i-1], (0, 0, 0), 3)
    print(pol[0])
    print(pol[1])
    print(pol[2])
    print(pol[3])
    ...
    for i in range(660, 760, 10):
        p = MyPoint(Fraction(780), Fraction(i))
        its = 0
        cp = p
        while True:
            np = makeReflectionPoint(cp, pol, False)
            if np is None:
                its = -1
                break
            its += 1
